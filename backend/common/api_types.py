...
class BodyFatInput(BaseModel):
    ...
    hip: Optional[float] = Field(None, ge=0, description="Hip circumference in cm (required for females)")
...
class BodyFatResponse(BaseModel):
    ...
    category: str = Field(..., description="Body fat category (e.g., essential, athletes, fitness, average, obese)")


# Macronutrient Calculator Input and Response Models
class MacrosInput(BaseModel):
    """Input for macronutrient calculation."""
    tdee: float = Field(..., gt=0, description="Total Daily Energy Expenditure (calories)")
    goal: str = Field(..., description="Goal: 'lose_weight', 'maintain', 'build_muscle'")
    diet_type: Optional[str] = Field(None, description="Diet type: 'standard', 'keto', 'paleo', 'vegetarian', 'vegan' (default: standard)")


class MacrosResponse(BaseModel):
    """Response for macronutrient calculation."""
    calories: float = Field(..., description="Daily calorie target (may be adjusted from TDEE)")
    ...
    carbs_percentage: float = Field(..., description="Carbohydrate percentage of total calories")
